...
OpponentPredictor::OpponentPredictor() : Node("opponent_predictor_node") {
  ...
  this->declare_parameter<bool>("visualize_grid", false);
  this->declare_parameter<bool>("visualize_obstacle", true);
  this->declare_parameter<bool>("visualize_opp", true);
  this->declare_parameter<bool>("visualize_opp_pose", false);
  this->declare_parameter<bool>("visualize_opp_bbox", false);
  this->declare_parameter<bool>("real_test", false);

  // --- Strings ---
  this->declare_parameter<std::string>("map_name", "levine_2nd");
  this->declare_parameter<std::string>("map_img_ext", ".png");
  this->declare_parameter<std::string>("track_file", "centerline.csv");
  this->declare_parameter<std::string>("inner_bound", "inner_bound.csv");
  this->declare_parameter<std::string>("outer_bound", "outer_bound.csv");

  // --- Numerics ---
  this->declare_parameter<double>("resolution", 0.05);
  this->declare_parameter<double>("grid_xmin", -10.0);
  this->declare_parameter<double>("grid_xmax",  10.0);
  this->declare_parameter<double>("grid_ymin", -10.0);
  this->declare_parameter<double>("grid_ymax",  10.0);
  this->declare_parameter<double>("grid_resolution", 0.1);
  this->declare_parameter<double>("plot_resolution", 0.1);
  this->declare_parameter<double>("grid_safe_dist", 0.25);
  this->declare_parameter<double>("goal_safe_dist", 0.25);
  this->declare_parameter<double>("cluster_dist_tol", 0.4);
  this->declare_parameter<int>("cluster_size_tol", 3);
  this->declare_parameter<double>("avoid_dist", 0.6);

  // --- Vectors (예: map origin [x, y, yaw]) ---
